...
| `packages/convex` (`@metacrdt/convex`) | Convex/core adapters, validators, Confect sidecar warning, and a component-owned protocol log |
...
1. **A map, not a migration.** This repo stays one reference implementation until
   ...
   too**: Convex has adapters plus the first component-owned protocol log; Forma
   is the runtime-neutral language package; runtime is harness-first (service
   contracts, memory target, localStorage target seed, BroadcastChannel transport
   seed, and p2p DataChannel transport);
