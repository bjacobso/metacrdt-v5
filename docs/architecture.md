...
| `packages/node` (`@metacrdt/node`) | open server-process target package with memory runtime wrapper and structural server-SQLite event/HLC/seq services passing shared conformance |
