...
| [`branding.md`](./branding.md)           | The name stack: Open Ontology as umbrella, ontology.run as hosted service, MetaCRDT as substrate, Forma as language, Schematics as workbench, Onboarded as proof | naming        |
...
| [`ontologyvm.md`](./ontologyvm.md)       | The OntologyVM naming exploration: category criteria, WorldVM comparison, MetaCRDT → OntologyVM → ontology.run stack, and homepage thesis | naming        |
| [`worldvm.md`](./worldvm.md)             | The full end-to-end WorldVM pitch: virtual machine for organizational reality, the World → Context → Thread hierarchy, the built-substrate evidence table, `@worldvm` package ecosystem, wedge products, and the ledger delta vs. `branding.md` | naming        |
