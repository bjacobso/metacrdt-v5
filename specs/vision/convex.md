...
> **MetaCRDT framing →** Convex is one _target_, not the substrate (see [`../reference/targets.md`](../reference/targets.md)). **Reversed cut:** "Effect → Convex validators" is superseded — Effect now lives at a target-neutral runtime-shape tier (Confect-like) above the targets ([`../reference/protocol.md`](../reference/protocol.md) §1.2). See [`metacrdt-alignment.md`](./metacrdt-alignment.md) §0, §3.
