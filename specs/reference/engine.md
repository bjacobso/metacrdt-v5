...
**The reference-implementation walkthrough: the model, the package graph, the
Convex runtime, and the query/write surface.**

This is the *what is true now* description of the running system. For the
normative protocol see [protocol.md](./protocol.md); for positioning and the
honest built-vs-frontier framing see [positioning.md](./positioning.md); for the
naming/layer map see [architecture.md](./architecture.md). The repository is the
canonical MetaCRDT reference implementation and package monorepo. It currently
runs on [Convex](https://convex.dev) as a centralized, reactive reference
runtime, with the pure convergence kernel already extracted as `@metacrdt/core`.
...
The normative protocol is [SPEC.md](./protocol.md).
...
- **`@metacrdt/node`** (`packages/node`) — open server-process target:
  ...
  without native database dependencies. It also exposes a shared SQL lifecycle
  plan for the runtime event/meta tables and indexes, plus a dependency-free
...
- Packages build with **tsdown** (powered by **Rolldown**) into `dist` ESM
  JavaScript plus declaration files. Package scripts share the root
  [`tsdown.config.ts`](../../tsdown.config.ts), with package-specific entries and
...
@metacrdt/collect     forms, collection, prompt-response
