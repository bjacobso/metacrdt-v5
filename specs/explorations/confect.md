# Confect — the whole app as Effect, via Confect
...
Read alongside [VISION.md](../vision/overview.md) (§6 config-as-code, §10 AI) and
[PLAN.md](../../PLAN.md) (the vision-vs-Convex assessment this doc partly revisits).

> **Status.** This started as a directional design sketch. As of the Goal 2 spike,
> Confect v8.0.0 is wired into this repo as a narrow sidecar: `confect/` defines a
> typed Effect Schema function group, `confect/_generated/*` is generated by a
> safe wrapper, and `convex/metacrdtConfect.ts` manually mounts the generated
> function beside the existing hand-written Convex backend. The recommendation is
> **adopt narrowly for `@metacrdt/convex` experiments; do not convert the
> reference app wholesale yet.**
...
- `confect/schema.ts` models the existing `transactions` and `factEvents` tables
  with Confect `DatabaseSchema` / `Table` definitions.
- `confect/metacrdt.spec.ts` declares `metacrdt.verifyEvents` as a public query
  with Effect Schema args/returns and two tagged errors:
  `UnknownEntity` and `InvalidProtocolEvent`.
- `confect/metacrdt.impl.ts` reads real `factEvents` through generated
  `DatabaseReader`, reconstructs MetaCRDT core events, and validates their
  `eventId`s with `@metacrdt/core.verifyId`.
- `scripts/confect-codegen-sidecar.mjs` runs `confect codegen` safely by pointing
  the CLI at a throwaway generated Convex target. This preserves
  `confect/_generated/*` without letting Confect overwrite this repo's real
  `convex/` tree.
- `convex/metacrdtConfect.ts` manually mounts
  `registeredFunctions.metacrdt.verifyEvents` as a normal Convex export.
- `convex/confect.test.ts` verifies the sidecar function through `convex-test`.

Installed:

- Runtime dependencies: `effect`, `@confect/core`, `@confect/server`.
- Dev dependency: `@confect/cli`.
- Not installed: `@confect/react` (no frontend call in this spike),
  `@confect/test` (npm peer-resolution friction with the existing
  `convex-test`; the mounted generated function is covered by normal
  `convex-test` instead).

What worked:

- Confect/Effect handlers run inside the Convex isolate.
- Effect Schema returns are strict; exact optional fields must be omitted, not
  returned as `undefined`.
- Typed errors cross the Convex boundary as `ConvexError.data`. The
  `UnknownEntity` test proves the caller can observe the decoded tagged shape.
- Generated registered functions can be mounted manually beside existing plain
  Convex functions.
- `@metacrdt/core` imports cleanly inside the Confect handler.

What bit:

- Raw `confect codegen` is not sidecar-safe for this repo. It treats the whole
  configured Convex functions directory as generated output: it rewrites
  `schema.ts` and removes function modules that are not represented in the
  Confect spec. That is reasonable for a Confect-owned app, but dangerous for
  this hand-written reference runtime.
- A safe wrapper can generate `confect/_generated/*`, but this is a workaround,
  not a package boundary. `@metacrdt/convex` should not require host apps to run
  raw Confect codegen against their production `convex/` tree.
- The Confect schema duplicates the relevant Convex table shape for the spike.
  That duplication is acceptable for evaluation, but a reusable target package
  needs generated or shared schema fragments.
...
`verifiable: true`, and `validEventId: true`.

Recommendation:

- **Adopt Confect narrowly** for `@metacrdt/convex` internals and typed boundary
  experiments.
- Keep `@metacrdt/core` pure and Effect-free.
- Do not migrate the current reference app wholesale.
- Do not run raw `confect dev` in this repo unless the whole `convex/` tree is
  intentionally handed to Confect.
- For Goal 3, extract `@metacrdt/convex` first, then decide whether Confect
  support is a subpath helper (`@metacrdt/convex/confect`) or a docs recipe.
