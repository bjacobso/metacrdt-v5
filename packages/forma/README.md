...
```ts
import { Reader, Evaluator, Type } from "@metacrdt/forma";
```

Subpaths are also exported for direct consumers:

```ts
import { parseManyToSExpr } from "@metacrdt/forma/reader";
import { evaluate, makePreludeLayer } from "@metacrdt/forma/evaluator";
import { inferSourceStr } from "@metacrdt/forma/type";
```
...
`@metacrdt/forma`. Onlang can remain a historical alias for the authoring idea,
but new code should import from `@metacrdt/forma`.
