...
export default function EntityDetail() {
  ...
  const detail = useClientQuery("entities.entityDetail", { e: id });
  const primaryType = detail?.types[0];
  const schema = useClientQuery(
    "attributes.typeSchemaAsOf",
    primaryType ? { type: primaryType } : "skip",
  );
  const startFlow = useClientMutation("flows.startFlow");
  const runAction = useClientMutation("actions.runAction");
