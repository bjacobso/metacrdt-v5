...
export default function ComponentEntity() {
  ...
  const runOwnedAction = useClientMutation("metacrdtComponent.runOwnedAction");
  const startOwnedFlow = useClientMutation("metacrdtComponent.startOwnedFlow");
  const issueOwnedOpenCollections = useClientMutation(
    "metacrdtComponent.issueOwnedOpenCollections",
  );
  const materializeOwnedCompliance = useClientMutation(
    "metacrdtComponent.materializeOwnedCompliance",
  );
  const entity = useClientQuery("metacrdtComponent.getOwnedCurrentEntity", {
    e: id,
  });
  const events = useClientQuery("metacrdtComponent.listOwnedEvents", {
    e: id,
    limit: 20,
  });
  const collections = useClientQuery("metacrdtComponent.listOwnedCollections", {
    subject: id,
    limit: 20,
  });
  const flowRuns = useClientQuery("metacrdtComponent.listOwnedFlowRuns", {
    subject: id,
    limit: 20,
  });
  const types =
  ...
  const actions = useClientQuery(
    "actions.actionsForType",
    primaryType ? { type: primaryType } : "skip",
  );
  const flows = useClientQuery(
    "flows.flowsForType",
    primaryType ? { type: primaryType } : "skip",
  );
  const compliance = useClientQuery(
