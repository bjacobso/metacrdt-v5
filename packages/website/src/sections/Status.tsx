...
export function Status() {
  return (
    <section className="site-section section-rule border-b border-black/10" id="status">
  ...
  return (
    <article className="grid-card p-4">
      ...
      <ul className="mt-3 p-0">
        {items.map((item) => (
          <li className="list-none border-t border-black/10 py-2.5 text-sm leading-5 text-muted" key={item}>
            {item}
          </li>
        ))}
      </ul>
